...
def casino_game(display, dette=1000000000, starting_money=20, mode="balanced"):
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                ...
                if state == "deal":
                    ...
                    x0    = CX - total // 2
                    for i in range(n):
                        r = pygame.Rect(x0 + i * (CARD_W + CARD_GAP), PLAYER_Y, CARD_W, CARD_H)
                        if r.collidepoint(mx, my):
        ...
        # ── États ───────────────────────────────────────────────────
        if state == "deal":
            msg = font_big.render("Pressez DISTRIBUER pour commencer.", True, COLOR_GRAY)
            ...
            tw    = font_mid.size(f"Mise actuelle : {casino.bet}  /  Portefeuille : {casino.wallet}")[0]
            ...
            draw_hand(screen, font_mid, font_suit,
                      pl_hand, CX, PLAYER_Y, CARD_W, CARD_H, CARD_GAP,
                      selected_set=selected, cursor_idx=active_card_cursor)
            ...
            draw_hand(screen, font_mid, font_suit, pl_hand, CX, PLAYER_Y, CARD_W, CARD_H, CARD_GAP)
