...
def matches(item, template):
    return (
        item._name == template["name"]
        and item._hp == template["hp"]
        and item._sh == template["sh"]
        and item._st == template["st"]
    )
...
def buy_item(player, template):
    ...
    if player._money < template["buy"]:
        return False
    player._money -= template["buy"]
    for item in player.bag:
        if matches(item, template):
            item._qty += 1
            return True
    player.bag.append(
        Item(template["name"], template["hp"], template["sh"], template["st"], 1)
    )
    return True
...
def sell_item(player, item):
    ...
    if item._qty <= 0:
        return False
    item._qty -= 1
    player._money += get_sell_price(item)
    return True
...
def shop(display, player):
    ...
    player doit exposer :
        player._money (int)         – argent disponible
    ...
    while True:
        ...
        # ── Solde ──────────────────────────────────────────────────
        lbl_money = f"Argent disponible :  $ {player._money}"
        ...
        # ════════════════════════════════════════════════════════════
            ...
            for i, t in enumerate(SHOP_STOCK):
                ...
                disabled = player._money < t["buy"]
                label = f"{t['name']}  $ {t['buy']}"
                draw_button(
                    display, label, bx, by, bw, bh, selected=i == sel, disabled=disabled
                )
        ...
        # ════════════════════════════════════════════════════════════
            ...
            if n == 0:
                ...
                if sel < n:
                    ...
                    detail = (
                        f"HP +{item._hp}  SH +{item._sh}  ST +{item._st}"
                        f"   (en stock : {item._qty})"
                    )
                    display.render_ascii(
                        detail, C_GREY, _cx(screen, detail), back_y + 50
                    )
